...
if (!environment) {
  console.error("Usage: node scripts/init-skill-replay.mjs <environment> [output-file]");
  process.exit(1);
}

const evalMatrix = loadEvalMatrix();
const outputPath = path.resolve(outputArg ?? path.join(evalResultsDir, `${environment}.json`));
