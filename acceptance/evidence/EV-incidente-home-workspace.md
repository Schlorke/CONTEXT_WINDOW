...
`C:\Users\harry` é, ao mesmo tempo, um repositório git e um workspace pnpm (`pnpm-workspace.yaml`,
`package.json` do projeto `gb-locacoes`). A pasta de trabalho externa
(`C:\Users\harry\AuditoriasExternas\...`) fica dentro dele.

A cópia do fixture legado (`sandbox\legacy-e2e\legacy-shop`) não tinha `pnpm-workspace.yaml`
próprio. Ao rodar `pnpm install` nela, o pnpm subiu a árvore, tratou `C:\Users\harry` como raiz do
workspace e executou a instalação do projeto `gb-locacoes`:

| Quando (-03) | Evidência |
| --- | --- |
| 2026-09-24 23:53–23:54 | 1.459 pastas em `C:\Users\harry\node_modules\.pnpm` com data desse intervalo; `.modules.yaml` `prunedAt: 2026-09-25 02:54 GMT` |
...
- Os scripts de ciclo de vida da raiz rodaram: `prepare` (`husky`, regravou `.husky/_`) e
  ...
  `C:/Projetos/OkGas/prisma/prisma.config.ts`, que não existe, então a geração falhou antes de
...
- O repositório `C:\Projetos\OkGas` não foi tocado (o arquivo de configuração citado não existe).
...
- `.prettierrc.json` e `.editorconfig` (`root = true`) na raiz: a cópia limpa dentro do diretório
  pessoal herdava o `.prettierrc` de `C:\Users\harry`.
- Os roteiros de teste externos passaram a checar a data de `C:\Users\harry\node_modules\.modules.yaml`
...
O store da área de testes não deve ser apagado antes de o `node_modules` de `C:\Users\harry` ser
refeito. Para voltar ao estado habitual, rode no diretório `C:\Users\harry`, com o store padrão, a
instalação normal do projeto `gb-locacoes` (`pnpm install`); o pnpm deve pedir para recriar o
